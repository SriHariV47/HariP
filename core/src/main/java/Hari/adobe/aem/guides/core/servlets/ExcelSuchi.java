...
@Component(service = Servlet.class, property = { "sling.servlet.paths=/bin/uploadexcel/asdf" })
public class ExcelSuchi extends SlingAllMethodsServlet {
	
	//download and shows path in respomse
    private static final long serialVersionUID = 1L;

    String getColumnAlphabet(int column) {
    ...
    void executeQuery( List<String> multiFieldslist ,List<String> headerFields, SlingHttpServletRequest request, SlingHttpServletResponse response) throws IOException {
    ...
    void addPropertyName(Map<String, String> propertiesMap, Node node) throws javax.jcr.RepositoryException {
        String Nameproperty = node.getProperty("name").getString();
        String fieldLabelproperty = node.getProperty("fieldLabel").getString();
        int lastSlashIndex = Nameproperty.lastIndexOf("/");	//slice method
        if (lastSlashIndex != -1) {
    ...
    public void addproperties(List<String> multiFieldslist ,Map<String, String> propertiesMap, SlingHttpServletRequest request, SlingHttpServletResponse response) throws IOException {
         ...
         try {
        	 ...
        	 //update properties in crxde
             String path = "/content/dam/excelfiles/PORTAL_SCHEMA (1).xlsx/jcr:content/metadata";
